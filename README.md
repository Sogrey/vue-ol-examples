...
- **位置信息栏示例**：[Example](https://sogrey.top/CodeSandbox/?page=https://sogrey.top/vue-ol-examples/examples/ol-Location-Bar.html)
...
- 自定义TileGrid瓦片网格，支持百度BD:09坐标系
- 实现百度地图特有的瓦片坐标系转换（Y坐标反转）
- 支持负坐标处理（M前缀），完整覆盖全球范围
- 配置19级详细分辨率数组，支持高精度地图显示
...
### ol-Location-Bar (public/examples/ol-Location-Bar.html)
...
- **坐标转换**：支持不同坐标系之间的转换显示（EPSG:3857到EPSG:4326）
- **多事件监听**：监听`pointermove`、`change:resolution`、`moveend`等地图事件
- **美观的UI设计**：底部状态栏布局，半透明背景，信息清晰展示
- **精确度控制**：经纬度显示精确到小数点后6位，角度显示到整数位
- **视图范围获取**：在地图移动结束后获取可视化区域的范围信息
...
- 监听多种地图事件获取实时状态信息：
  - `pointermove`：鼠标移动时更新经纬度和方向
  - `change:resolution`：分辨率变化时更新缩放级别
  - `moveend`：地图移动结束后获取视图范围
- 自定义CSS样式实现美观的底部状态栏界面
- 使用`view.calculateExtent()`获取可视化区域范围
